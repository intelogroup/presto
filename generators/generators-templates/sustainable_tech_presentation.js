...
class SustainableTechGenerator {
    ...
    async loadAvailableAssets() {
        ...
        const assetsDir = path.join(__dirname, '../../assets-images-png');
        
        // Load Lucide PNG icons
        try {
            const lucideDir = path.join(assetsDir, 'lucide', 'general');
            if (pathExists(lucideDir)) {
                const files = fs.readdirSync(lucideDir).filter(file => file.endsWith('.png'));
                this.assets.lucidePng = files.map(file => path.join(lucideDir, file));
                console.log(`✅ Loaded ${files.length} Lucide PNG`);
            }
        } catch (error) {
            console.warn('Could not load Lucide PNG:', error.message);
        }
        
        // Load Simple Icons PNG
        try {
            const simpleIconsDir = path.join(assetsDir, 'simpleicons', 'brand');
            if (pathExists(simpleIconsDir)) {
                const files = fs.readdirSync(simpleIconsDir).filter(file => file.endsWith('.png'));
                this.assets.simpleIconsPng = files.map(file => path.join(simpleIconsDir, file));
                console.log(`✅ Loaded ${files.length} Simple Icons PNG`);
            }
        } catch (error) {
        ...
        try {
            const svgrepoDir = path.join(assetsDir, 'svgrepo-icons-graphics');
            if (pathExists(svgrepoDir)) {
                const files = fs.readdirSync(svgrepoDir).filter(file => file.endsWith('.png'));
                this.assets.svgrepoIconsPng = files.map(file => path.join(svgrepoDir, file));
                console.log(`✅ Loaded ${files.length} SVG Repo Icons PNG`);
            }
        } catch (error) {
        ...
        // Load DevIcons PNG
        try {
            const devIconsDir = path.join(assetsDir, 'devicons', 'tech');
            if (pathExists(devIconsDir)) {
                const files = fs.readdirSync(devIconsDir).filter(file => file.endsWith('.png'));
                this.assets.devIconsPng = files.map(file => path.join(devIconsDir, file));
                console.log(`✅ Loaded ${files.length} DevIcons PNG`);
            }
        } catch (error) {
